...
export default async function RosterPage({ params }: RosterPageProps) {
  ...
  return (
    <main>
      <ContentSection
        heading={`${gameConfig.displayName} Rosters`}
        description="View registered player profiles grouped by school teams and squad divisions"
        theme="dark"
      >
        <div className="max-w-6xl mx-auto space-y-12">
          {teamGroups.length === 0 ? (
            <div className="text-center p-12 text-slate-500 text-sm bg-slate-900/20 rounded-2xl border border-slate-800/60">
              No active rosters registered for this game yet.
            </div>
          ) : (
