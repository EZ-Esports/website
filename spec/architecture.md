...
- **Destructive row actions are trash-can icon buttons.** Delete/remove triggers in staff tables are icon-only `FiTrash2` buttons, never text. `ConfirmDeleteButton` (`window.confirm` + server-action form) renders the icon itself; its `label` is the accessible name and `title` (pass a specific one, e.g. `Delete school Foo`) (required). Every trash button wears one faint-red look (`bg-red-950/10 border-red-900/30 text-red-400`, deeper on hover), the Applications tab's, never gray; there is no neutral variant or `tone` prop. Styles live in `admin/styles.ts`: `deleteIconBtn` is a 32px visible square, hit area padded to ~42px by an `after:-inset-1.5` pseudo-element (6px overhang: keep >=6px of gap/padding around it, e.g. `pr-2` on a bare actions cell inside `overflow-x-auto`), visible red focus ring. `deleteIconBtnCompact` is the same colours without size/overhang (add `p-1.5`) for the dense hover-revealed roster/league rows and tiles. The confirm dialog is the native `window.confirm`, so there is no in-dialog "Delete" button to style. Row actions must not be hover-gated (`opacity-0 group-hover:*`): touch has no hover. The Standings Archive rows were made permanently visible; Roster Explorer and League Setup rows still use the hover-reveal pattern (see [open-threads.md](open-threads.md)).
