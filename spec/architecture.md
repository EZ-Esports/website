...
Applicant resumes are the one **private** bucket: `staff-resumes` (`STAFF_RESUME_BUCKET`, [spec-004](spec-004-staff-application-resume.md)). The public staff-apply route writes to it with the service client only after all validation passes (including the `%PDF-` magic bytes), under a random `<uuid>.pdf` key, and stores the key in `staff_applications.resume_storage_key`, never a URL. The only read path is `GET /admin/applications/staff/[id]/resume`, which is gated by `getStaffForAdminSection('/admin/applications')` and 303-redirects to a 60 s signed URL.
...
- **Dropdown chevron inset.** Every native single-choice `<select>` is styled by one unlayered rule in `globals.css` (`select:not([multiple]):not([size])`): `appearance: none` plus a themed data-URI chevron (`--select-chevron`, dark in `:root`, `.theme-light` override) placed `1rem` from the right edge with `2.5rem` right padding, so the "v" never hugs the edge and text (ellipsised) never runs under it. It is deliberately unlayered so it beats Tailwind `px-*`/`pr-*` utilities. Do not add per-select arrow icons or `appearance-none`; a cramped control tunes `--select-chevron-inset` / `--select-chevron-space` on itself (Match Fixtures status select: `0.625rem` / `2rem`). The one RAC dropdown, `SeasonSelect`, uses an inline `FiChevronDown` with `pr-4` to match.
- **Row actions share one 32px-tall family.** Edit = `FiEdit2` pen icon button, save = TEXT button ("Save", or "Saving…" where the site already had a pending label), cancel = quiet TEXT button (an `FiX` icon only for the Gallery card's edit/cancel toggle), delete = faint-red `FiTrash2` icon button. Edit and save carry the "white glow": soft-white `text-foreground` on `bg-surface-raised`, `hover:bg-line`. There is no literal box-shadow; it is the Leadership Manager's original Edit look, extracted once as `whiteGlow` in `admin/styles.ts` (`editIconBtn`, `saveBtn`, `cancelBtn`, `cancelIconBtn`, `deleteIconBtn`) so it cannot drift. Delete is faint red (`bg-red-950/10 border-red-900/30 text-red-400`, the Applications tab's look; never gray). Use `RowIconButton` (`kind="edit" | "cancel"`, required `label` naming the target) for icon buttons, the exported class for a `Link`, and `ConfirmDeleteButton` (`window.confirm` + server-action form, required `label`) for confirmed deletes. An edit trigger that toggles an inline form switches to the cancel X with `aria-expanded`. Icon geometry: hit area padded to ~42px by an `after:-inset-1.5` pseudo-element (6px overhang): keep `gap-2` between buttons, >=6px padding around the cluster, and `pr-2` on a bare `text-right` cell inside `overflow-x-auto`. Text buttons have no overhang. `deleteIconBtnCompact` is only for the corner trash on roster/team tiles. Deliberately left with their own text styling: descriptive form-footer submits ("Save Changes", "Save as Draft", "Create", "Add", "Register"). The confirm dialog is the native `window.confirm`, so there is no in-dialog "Delete" button to style. Row actions must not be hover-gated (`opacity-0 group-hover:*`): touch has no hover. Standings Archive rows were made permanently visible; Roster Explorer and League Setup rows still use the hover-reveal pattern (see [open-threads.md](open-threads.md)).
...
Agent skills (`implement`, `review-loop`, `ui`, `uiux`, `create-artifact`, `cleanup`) are repo artifacts for AI-assisted development, not runtime features. **`.claude/` and `.gemini/` are kept in parallel sync**. The core agent workflows (`implement`, `review-loop`, and `orchestrator`) mandate that **`spec/` is the 1st point of reference** for historical context and architecture, and **git history is strictly the last point of reference**. Implementers update `spec/` directly in the PR diff, and reviewers verify spec updates in the review loop. Implement skill uses isolated worktrees (`.gemini/worktrees/` or `.claude/worktrees/`) per PR. `CLAUDE.md` ends with a Next.js-generated `<!-- BEGIN:nextjs-agent-rules -->` block rewritten by `next dev` (`node_modules/next/dist/server/lib/generate-agent-files.js`); dropping it from a diff only recreates the uncommitted change.
