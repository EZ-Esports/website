...
| [`spec-004`](spec-004-staff-application-resume.md) | Staff Application Resume Upload, Volunteer Acknowledgement, and Recruiting Intro | `fix/staff-apply-tidy` (PR pending) | 2026-09-25 | In Progress | Required PDF resume in the private `staff-resumes` bucket (magic-byte checked, 4 MB cap, admin-only 60 s signed URL), optional LinkedIn and "Other links", unpaid-volunteer acknowledgement (`details` v3), recruiting intro with mission statement. Migration `0036` and bucket still to be set up by hand. |
