...
| `/apply/staff` | Staff application (`d6518cc`). Nine divisions (Software Engineering, Systems Engineering, Data Science, Marketing, Operations, Development, Productions Crew, Legal, Game Regulations), where Game Regulations Division asks a required open-answer follow-up ("Which game director position are you interested in?", 500-character cap). The why-join answer shows its character limit (1,500); the Game Regulations answer (500) and Other links (1,000) are capped without a notice. Required PDF resume (private `staff-resumes` bucket), optional LinkedIn (linkedin.com hosts only) and "Other links", required unpaid-volunteer acknowledgement, recruiting intro with the mission statement, an "Unpaid volunteer" tag, and no future-pay wording ([spec-004](spec-004-staff-application-resume.md)). Admin opens resumes via a 60 s signed URL. |
| `/rules` | League Rulebook & Code of Conduct (`6732658`). Linked from apply consent. |
| `/privacy` | Privacy policy + Privacy erasure intake modal/action (`a1aeca6` PR #142 / #103). Covers phone, Discord, and staff application resumes: private storage, links for authorized staff and administrators that expire shortly after creation, and deletion when an application is declined or on request (a manual runbook for now) ([spec-004](spec-004-staff-application-resume.md)). **No `/terms` on this branch.** |
