...
| `/apply/staff` | Staff application (`d6518cc`). Seven divisions, where Game Regulations Division asks a required open-answer follow-up ("Which game director position are you interested in?", up to 500 characters). Required PDF resume (private `staff-resumes` bucket), optional LinkedIn (linkedin.com hosts only) and "Other links", required unpaid-volunteer acknowledgement, recruiting intro with the mission statement, an "Unpaid volunteer" tag, and no future-pay wording ([spec-004](spec-004-staff-application-resume.md)). Admin opens resumes via a 60 s signed URL. |
| `/rules` | League Rulebook & Code of Conduct (`6732658`). Linked from apply consent. |
| `/privacy` | Privacy policy + Privacy erasure intake modal/action (`a1aeca6` PR #142 / #103). Covers phone, Discord, and staff application resumes: private storage, reviewer-only expiring links, no automatic deletion, deletion on request ([spec-004](spec-004-staff-application-resume.md)). **No `/terms` on this branch.** |
