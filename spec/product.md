...
| `/apply/staff` | Staff application (`d6518cc`). Nine divisions (Software Engineering, Systems Engineering, Data Science, Marketing, Operations, Development, Productions Crew, Legal, Game Regulations), where Game Regulations Division asks a required open-answer follow-up ("Which game director position are you interested in?", up to 500 characters). Every free-text answer shows its character limit (why-join: 1,500). Required PDF resume (private `staff-resumes` bucket), optional LinkedIn (linkedin.com hosts only) and "Other links", required unpaid-volunteer acknowledgement, recruiting intro with the mission statement, an "Unpaid volunteer" tag, and no future-pay wording ([spec-004](spec-004-staff-application-resume.md)). Admin opens resumes via a 60 s signed URL. |
