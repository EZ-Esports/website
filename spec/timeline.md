...
- **23 Sep — Admin sidebar pinned.** `AdminShell.tsx` sidebar is now `sticky top-0 h-screen self-start` with a scrollable nav (`min-h-0 overflow-y-auto`), so Public Site / Sign Out stay at the bottom of the viewport instead of following tall tab content. Only the page scrolls; there is no mobile drawer (sidebar was never responsive).
