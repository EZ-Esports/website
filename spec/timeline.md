...
- **11–12 Sep — security/legal.** Dependabot #59–#64 (`9d40278` #128, `0463b43` #130). HTTP headers `0442af1` (#129) — **no CSP**. **`/rules` republished** `6732658` (#132) — this is the live page, not the Aug 17 draft (`615b340` is a parallel-branch draft, not an ancestor). Markdown URL XSS + ReDoS `926b978` (#134).
...
- **19–20 Sep — Security, Storage, Auth, and Timezone Hardening Sprint.**
  ...
  - **PR #180 `2e3e5d8` (#146, [spec-002](spec-002-database-safety-gates.md))** — Database safety gates. Applied `assertSeedTargetAllowed()` across `db/migrate.ts`, `drizzle.config.ts`, `db/seed-owner.ts`, `db/backfill-leadership.ts`, and `app/lib/db/seed-phase2.ts`. Deprecated UUID-churning delete-and-reinsert `db/seed.ts`. Gitignored `sharepoint/**/*.csv`.
  - **PR #181 `37515cd` (#156, [spec-003](spec-003-staff-role-lock-and-hierarchy.md))** — Auth lock-then-re-read. Fixed TOCTOU privilege escalation in `app/(admin)/admin/team/actions.ts`: acquired `pg_advisory_xact_lock(STAFF_REVOCATION_LOCK_KEY)` and re-read live database roles inside `tx` for role assignments, edits, and invite revocations.
  - **PR #182 `bbae922` (#152, [spec-005](spec-005-scoped-storage-keys-and-cleanup.md))** — Scoped storage keys. Stopped trusting client-supplied deletion keys. Enforced section permissions (`Permissions.MANAGE_*`) in `app/api/upload/route.ts` and stored files deterministically as `${section}/${entityId}/${timestamp}.${ext}` with path traversal protection. Derived deletes server-side in gallery, schools, sponsors, and leadership actions. Added `db/storage-clean.ts` cleanup script.
  - **PR #183 `1f694de` (#154, [spec-006](spec-006-gallery-reorder-derived-state.md))** — Gallery reorder derived state. Eradicated dual source-of-truth bug in `GalleryManagerClient.tsx`: eliminated duplicated React state and brittle 30-line `useEffect`, storing only uncommitted `draftOrder: string[] | null` and deriving card display from server props. Enforced `pg_advisory_xact_lock` + full ID list validation in `updateGalleryImagesOrder`.
  - **PR #184 `41270c5` (#148, [spec-004](spec-004-eastern-time-schedule-normalization.md))** — Eastern Time schedule & status normalization. All league games canonicalized to America/New_York via `parseEastern` in `app/lib/dates.ts`. Explicit "ET" UI labeling, calendar grid timezone anchoring, and status normalization (forfeits treated as completed, draws rendered as `D <score>`).
- **23 Sep — Admin sidebar pinned ([spec-001](spec-001-admin-sidebar-pinning.md), PR #187 `875bf56`).** Pinned staff sidebar to viewport height (`sticky top-0 h-dvh self-start`) with static nav (`min-h-0 overflow-hidden`) and persistent footer links. Only the page scrolls; there is no mobile drawer.
- **24 Sep — Admin Match Fixtures tightened to fit.** The `/admin/matches` fixtures table (four columns, `px-6` cells, fixed `w-28` team names) had a min-content width (~860px) larger than its `lg:col-span-2` grid column beside the sidebar (~456px at 1024, ~625px at 1280), so it needed horizontal scrolling. Same table, tighter: `px-2 py-3` cells, `w-16` team names (`2xl:w-28`), narrower score inputs and buttons; min-content is now ~590px (~615px while a row shows "Saving…"), so it fits from about 1230px inner viewport width up (about 1250-1270px for a Windows window, which loses ~15px to the scrollbar gutter) (`overflow-x-auto` kept as a safety net below that, incl. the wider "Saving…" label). Not achievable at 1024px with the original table look; stacking the grid until `xl` would be the layout fix if needed. Score inputs and status select gained `aria-label`s. Small UI fix, no spec-00x.
- **24 Sep — Dropdown chevron spacing.** Native `<select>` arrows sat almost flush with the right edge. One global rule in `globals.css` now gives every single-choice select a themed custom chevron inset `1rem` from the right (`2.5rem` right padding, ellipsis truncation); `TeamsFilterClient` dropped its hand-placed SVG chevrons and `SeasonSelect` (RAC) went to `pr-4`. The Match Fixtures status select tightens the inset (`0.625rem` / `2rem`) to protect that table's width budget: min-content grows only about 4px (~594px, ~619px while a row shows "Saving…", vs the ~625px column at 1280). Convention recorded in [architecture.md](architecture.md) (Frontend). Small UI fix, no spec-00x.
- **24 Sep — Trash-can delete buttons and always-visible Standings Archive actions.** Every text Delete/Remove trigger in the staff area (`ConfirmDeleteButton` callers: news, schools, sponsors, gallery, leadership, applications, standings, match fixtures; plus Team Manager's Delete Role) is now a `FiTrash2` icon button with a specific `aria-label`/`title`. The Standings Archive row actions (edit and delete) lost their hover/focus-only reveal so they are always visible and reachable on touch. Match Fixtures gets narrower: the 66px Delete text button became a 32px icon, so the table is roughly 30px narrower and still fits the ~625px column at 1280. Convention recorded in [architecture.md](architecture.md) (Frontend). Small UI fix, no spec-00x.
- **24 Sep — Trash buttons unified to faint red.** All trash-can delete buttons in the staff area now share the Applications tab's faint-red look instead of gray: `ConfirmDeleteButton` lost its neutral variant and `tone` prop (one `deleteIconBtn` export), and the older hand-rolled trash buttons in League Setup and Roster Explorer (rows and team/roster tiles) use a compact `deleteIconBtnCompact` at their existing size. Colour-only change, no width impact on Match Fixtures. Convention in [architecture.md](architecture.md) (Frontend). Small UI fix, no spec-00x.
- **24 Sep — Row-action buttons made consistent.** Every staff-area Edit is now the `FiEdit2` pen icon with the Leadership Manager Edit button's soft-white "glow" look; every inline "Save" stays text but wears the same glow (32px tall so it lines up with the icons), inline-form Cancel is a quiet text button, and delete stays the faint-red trash, all from one place in `admin/styles.ts` (`RowIconButton` for icon buttons). Covers Match Fixtures, Standings, Schools, Sponsors, Leadership, Gallery cards, News, League Setup, Roster Explorer and the Content editor. Match Fixtures min-content is about 575px idle and about 595px while a row shows "Saving…", inside the ~625px column at 1280. Descriptive form-footer submits stay as they were. Convention in [architecture.md](architecture.md) (Frontend). Small UI change, no spec-00x.
