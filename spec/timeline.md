...
- **23 Sep — Admin sidebar pinned.** `AdminShell.tsx` sidebar is now `sticky top-0 h-dvh self-start` with a non-scrolling nav (`min-h-0 overflow-hidden`), so Public Site / Sign Out stay at the bottom of the viewport instead of following tall tab content. Only the page scrolls; there is no mobile drawer (sidebar was never responsive).
