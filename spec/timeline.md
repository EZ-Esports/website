...
- **19–20 Sep — Security, Storage, Auth, and Timezone Hardening Sprint.**
  ...
  - **PR #184 `41270c5` (#148)** — Eastern Time schedule & status normalization. All league games canonicalized to America/New_York via `parseEastern` in `app/lib/dates.ts`. Explicit "ET" UI labeling, calendar grid timezone anchoring, and status normalization (forfeits treated as completed, draws rendered as `D <score>`).
