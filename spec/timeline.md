...
- **19–20 Sep — Security, Storage, Auth, and Timezone Hardening Sprint.**
  ...
  - **PR #181 `37515cd` (#156)** — Auth lock-then-re-read. Fixed TOCTOU privilege escalation in `app/(admin)/admin/team/actions.ts`: acquired `pg_advisory_xact_lock(STAFF_REVOCATION_LOCK_KEY)` and re-read live database roles inside `tx` for role assignments, edits, and invite revocations.
