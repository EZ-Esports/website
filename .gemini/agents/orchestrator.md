...
inside its own workspace (e.g. `Workspace: "share"`), or reads the diff purely through git plumbing
(`git diff`, `git show`, `gh pr diff`) — both stay entirely clear of the main
checkout (the repo root, not a `.gemini/worktrees/` path). The main checkout
...
state the exact workspace path (or commit range, if no workspace exists yet)
...
1. **Consult `spec/` and scope work.** Read `spec/` first for historical milestones,
   ...
   units where possible — independent units get their own workspace and can
...
2. **Implement & update spec.** For each unit, load the `implement` skill and delegate the
   ...
   `spec/open-threads.md`) directly in the PR diff. Spawn a subagent with `Workspace: "share"` when running
   multiple units in parallel; work inline in the current workspace/worktree for a
   single unit.
3. **Review.** Once a unit's implementation is complete, load the `review-loop`
   skill and spawn an independent review pass, handing it the unit's workspace
...
4. **Fix and verify.** If the review reports findings, delegate the fixes,
   ...
   again handed the workspace path — a different subagent from the one that
...
6. **Merge and clean up.** Once all units are clean, load the `cleanup`
   ...
   and workspaces, and verify `spec/` is fully up to date on `main`.
