...
1. **The main checkout is strictly read-only.** All audit passes, searches, and test executions must operate inside an isolated workspace/worktree or inspect git diffs via plumbing (`git diff`, `git show`, `gh pr diff`).
