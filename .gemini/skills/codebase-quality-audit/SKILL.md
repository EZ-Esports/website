...
1. **Isolated Worktree Execution**: Execute all audit passes, searches, and test suites inside an isolated workspace/worktree or inspect git diffs via read-only git plumbing (`git diff`, `git show`, `gh pr diff`). Keep the main repository checkout pristine and reserved for deliberate merges.
