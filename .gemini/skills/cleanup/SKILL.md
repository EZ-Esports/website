...
1. Confirm the target branch is actually merged into `main`: `git branch --merged main` should list it. Treat branches missing from that list as still in progress and leave them alone.
2. If there's unrelated uncommitted work sitting on `main` (check with `git status` before touching anything), protect it first: `git stash push -u`, do the rest of this cleanup, then `git stash pop` at the end to restore it.
