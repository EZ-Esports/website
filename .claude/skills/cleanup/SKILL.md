...
in-progress worktrees and branches.
...
5. Remove the worktree: use `ExitWorktree` for an active session worktree,
   or `git worktree remove` for a completed worktree directory.
6. Confirm `spec/` is up to date: verify `spec/timeline.md` and related spec
   files on `main` reflect the merged PR and its architectural impact.
7. Confirm the stash was restored (if stashed in step 2) and that `git status`
   is clean before finishing.
